...
When a commit has bookmarks or tags, storing a revision link prompts for a
change ID, commit ID, bookmark, or tag.  Otherwise it stores a full change ID
by default.  Use ~C-u~ with ~org-store-link~ for an immutable commit ID without
prompting, or ~C-u C-u~ to preserve the revision expression at point.
