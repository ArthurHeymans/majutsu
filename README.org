#+html: <p align="center"><a href="https://majutsu.org"><img src="docs/site/public/og.png" alt="Majutsu — Shape history without leaving Emacs" width="720"></a></p>
...
integration supports colocated jj/Git repositories, including sibling JJ
workspaces without a ~.git~ directory: Majutsu initializes Forge from the
underlying Git worktree.  Forge sections also depend on a configured,
...
When [[https://github.com/oantolin/embark][Embark]] is loaded, Majutsu automatically adds actions for workspace
...
** Optional Org Integration

When Org is loaded, Majutsu registers link types for repositories, revisions,
filtered logs, and historical file locations.  Use ~org-store-link~ in a Majutsu
buffer and ~org-insert-link~ in an Org buffer.

| Link type      | Target                                  |
|----------------+-----------------------------------------|
| ~majutsu:~       | Repository log                          |
| ~majutsu-rev:~   | Revision diff                           |
| ~majutsu-log:~   | Log filtered by a JJ revset             |
| ~majutsu-file:~  | File at a revision and line             |

When a commit has at least one bookmark or tag, storing a revision link prompts
for its change ID, commit ID, and each available symbolic name.  This also
applies when only bookmarks or only tags are present.  Otherwise Majutsu stores
a full change ID by default.  Use ~C-u~ with ~org-store-link~ for an immutable
commit ID without prompting, or ~C-u C-u~ to preserve the revision expression
at point.
...
- Emacs 29.1+
- [[https://github.com/jj-vcs/jj][Jujutsu (jj)]] v0.41.0+
- [[https://magit.vc/][magit]] 4.4.0+, [[https://github.com/magit/transient][transient]] 0.8.7+, [[https://github.com/emacs-compat/compat][compat]]
- [[https://github.com/minad/consult][consult]] 1.0+ and [[https://github.com/alphapapa/plz.el][plz]] 0.9.1+ (Gerrit completion)
- [[https://github.com/magit/with-editor][with-editor]] (pulled in via magit; message editing)
