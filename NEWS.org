...
- Commit signing transients ::
  Add selection-aware =jj sign= and =jj unsign= frontends.  Both accept
  repeatable revision selections; Sign also exposes an explicit signing key
  with backend-aware completion for GPG, GPGSM, and SSH.

- Whole-file patch selections ::
  Split, Squash, and Restore can select hunkless changes, including binary
  files, with ~F~.  Structured jj metadata supplies and verifies the file
  operations and paths before they are replayed.
...
- Git push makes multi-bookmark selection explicit ::
  The bookmark prompt is plural and accepts multiple bookmark names or
  patterns, producing one repeated =--bookmark= argument per selection.

- Metaedit supports visual multi-revision selection ::
  The Metaedit transient preselects revisions from the region, point, or =@=,
  and ~r~ can toggle additional point or region revisions with selection
  overlays.  The old single-revision restriction has been removed to match
  current jj.
...
- ~majutsu-diff-refresh~ was removed ::
  Use ~d~ to execute the transient with the selected arguments, or ~g~ in an
  existing diff buffer to refresh that buffer.
