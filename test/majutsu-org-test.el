...
(ert-deftest majutsu-org-revision-store/stores-revision-at-point ()
  (with-temp-buffer
    ...
    (let ((majutsu-org-revision-storage 'symbolic)
          (majutsu-org-prompt-for-symbolic-revisions nil)
          properties)
      (cl-letf (((symbol-function 'majutsu-revision-at-point)
                 (lambda () "change-id"))
                ((symbol-function 'org-link-store-props)
...
(ert-deftest majutsu-org-select-stored-revision/offers-bookmarks-and-tags ()
  (let ((majutsu-org-prompt-for-symbolic-revisions t)
        (majutsu-org-revision-storage 'change-id)
        offered)
    (cl-letf (((symbol-function 'majutsu-org--revision-metadata)
              ...
              ((symbol-function 'completing-read)
               (lambda (_prompt choices &rest _)
                 ...
                 "Bookmark: main")))
      (should (equal (majutsu-org--select-stored-revision "source") "main"))
      (should (equal (cdr (assoc "Change ID: change-id" offered))
                     "change-id"))
      (should (equal (cdr (assoc "Commit ID: commit-id" offered))
                     "commit-id"))
...
(ert-deftest majutsu-org-select-stored-revision/skips-prompt-without-symbols ()
  (let ((majutsu-org-prompt-for-symbolic-revisions t))
    (cl-letf (((symbol-function 'majutsu-org--revision-metadata)
               (lambda (_) '(:change-id "change" :commit-id "commit"
                             :bookmarks nil :tags nil)))
              ((symbol-function 'majutsu-org--canonical-revision)
               (lambda (_revision _kind) "change"))
              ((symbol-function 'completing-read)
               (lambda (&rest _) (ert-fail "Unexpected prompt"))))
      (should (equal (majutsu-org--select-stored-revision "source")
                     "change")))))

(ert-deftest majutsu-org-select-stored-revision/prefix-bypasses-prompt ()
  (let ((majutsu-org-prompt-for-symbolic-revisions 'always)
        (current-prefix-arg '(4)))
    (cl-letf (((symbol-function 'majutsu-org--canonical-revision)
               (lambda (_revision kind)
                 (should (eq kind 'commit-id))
                 "commit"))
              ((symbol-function 'majutsu-org--revision-metadata)
               (lambda (&rest _) (ert-fail "Unexpected metadata query"))))
      (should (equal (majutsu-org--select-stored-revision "source")
                     "commit")))))
